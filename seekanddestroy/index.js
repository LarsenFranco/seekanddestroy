...
//!Vaciador no funciona. No se porque
//  const vaciador = function (){
//   $('#search').text("");
//   $('#amigo').text("");
//   $('#inputDelete').text("");
//  $('#sucess').text("");
    ...
    muestraAmigos();
     ...
     for (const per of data) {
         if(input.value==per.id){ 
            $('.card-header').text("Resultado:")
            $('#card-title').text("Nombre "+per.name)
            $('#card-id').text("Identificacion: "+per.id)
            $('#card-age').text("Edad: "+per.age)
            $('#card-email').text("Email: "+per.email)
            }
     }
     })
 }
